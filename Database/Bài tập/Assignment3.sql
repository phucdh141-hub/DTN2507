...
With ste_MaxFullName AS (
SELECT MAX(length(FullName)) MaxName FROM `Account`
)
SELECT * FROM `Account` WHERE DepartmentID = 3 AND LENGTH(FullName) = (SELECT MaxName FROM ste_MaxFullName);
...
SELECT COUNT(*) As `User` FROM `Account` WHERE DepartmentID = 2;
